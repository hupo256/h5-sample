...
class Protocol extends React.Component {
  ...
  handelSubmit = () => {
    ...
    const { linkManId, type } = location.state
    const _barcode = location.state.barcode
    const { kitType } = this.state
    if (kitType === 'INTESTINE_BIND') {
      history.push(`/binding/question?linkManId=${linkManId}&barCode=${_barcode}&type=${kitType}`)
    } else {
      API.bindCollectorUserKit({ barcode: _barcode, linkManId, type }).then(res => {
        if (!res.code) {
          this.trackPointSampleBindComplete({ ...location.state, ...barcode })
          const { linkManId, bindUserName } = location.state
          this.saveLastUserLindManId({ linkManId, userName: bindUserName, barCode: _barcode }, '/binding/binding-success', res.data)
        }
      })
    }
  ...
  saveLastUserLindManId = (obj = {}, url, data) => {
    ...
    linkManId && API.saveLastUserLindManId({ linkManId, noloading: 1 }).then(res => {
      ...
      if (!code) {
        if (kitType !== 'INTESTINE_BIND') {
          upLindManId(obj)
          setSetssion('bindSuccess', { linkManId: res.data.linkManId, barcode: barCode })
        }
      } else {
        setSetssion('bindSuccess', { linkManId, ...barcode })
      }
      if (kitType === 'INTESTINE_BIND') {
        history.push(`/binding/question?linkManId=${linkManId}&barCode=${barCode}`)
      } else {
        history.push({
          pathname: url,
          state: { ...data }
  ...
  render() {
    const { showTips, running, kitType } = this.state
    console.log(kitType)
    return (
      <Page title='知情同意书'>
        <div>
