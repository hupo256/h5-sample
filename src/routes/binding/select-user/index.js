...
class SelectUser extends React.Component {
  ...
  chooseUser = (item, index) => {
    ...
    if (limitLinkManId !== -1 && limitLinkManId === id) { //有值时，则只能选择匹配的
      ...
      this.setState({ ...item, index }, () => this.gotoNextPage())
    } else { //为 -1时，作下一步限定
      ...
      API.valiDation({ barcode: barcode, linkmanId: id }).then(res => {
        ...
        if (code !== 0) {
          ...
          this.setState({ relationId, userName, birthday, sex, index, id }, () => this.gotoNextPage())
        }
      })
    }
  }
  ...
  render() {
    const { list, index, addNewFlag, showTips, linkManType } = this.state
    const { history } = this.props
          ...
          <div className={`${styles.userBox} ${barcode ? '' : styles.pt120}`}>
            ...
            {addNewFlag &&
              <div
                ...
                onClick={() => { history.push(`/binding/bind-user${barcode ? '?barcode=' + barcode : '?msg=1'}&linkManType=${linkManType}`) }}>
                <img src={addnew} />
                <span>新建检测者</span>
              </div>}
          </div>
