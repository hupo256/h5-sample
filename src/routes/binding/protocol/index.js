...
class Protocol extends React.Component {
  ...
  handelSubmit = () => {
    ...
    API.bindCollectorUser({ ...location.state, ...barcode, ...jdAccountNum, ...thirdPartyOrderNo, ...params }).then(res => {
      if (!res.code) {
        ...
        this.saveLastUserLindManId({ linkManId, userName: bindUserName }, '/binding/binding-success', res.data)
      }else{
        Toast.info(res.msg)
      }
    })
  ...
  saveLastUserLindManId = (obj = {}, url, data) => {
    ...
    linkManId && API.saveLastUserLindManId({ linkManId, noloading: 1 }).then(res => {
      const { code } = res
      let linkId = linkManId;
      if (!code) {
        upLindManId(obj)
        setSetssion('bindSuccess', { linkManId: res.data.linkManId, ...barcode })
      } else {
