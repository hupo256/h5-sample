  ...
  componentDidMount() {
    ...
    API.myInfo({ noloading: 1, clientType: `${ua.isAndall() ? 'app' : ''}` }).then(res => {
      ...
      if (!code) {
        ...
        // ua.isAndall() || this.props.history.push(`/login?url=/binding${search}`)
      ...
      API.listOptionalLinkManKit({ barcode: bcode, type: _type }).then(res => {
        ...
        if (!code) {
          data.list ? history.push({
            pathname:`/select-user-kit`,
            state:{ data, type: type || _type }
          }) : history.push(`/bind-user-kit?type=${type || _type}`)
        }
      })
    } else {
        ...
        API.preBindCollectorUserKit({ barcode: barCode, type }).then(res => {
          ...
          if (!code) {
            ...
            if (limitType === 3) {
              ...
              this.props.history.push(`/bindOnly?barCode=${barCode}&type=${type}`)
              return
            }
        ...
        API.preBindCollectorUser({ nomsg: 1, barCode }).then(res => {
          ...
          if (!code) {
            ...
            if (limitType === 3) {
              ...
              this.props.history.push(`/bindOnly?barCode=${barCode}`)
              return
            }
                    ...
                    <div className={`flex ${styles.videoBox}`}>
                      <div
                        ...
                        <video
                          ...
                          controls src='//images.dnatime.com/app/source/video/8a87da63-804f-40c4-937c-293870df1a38.MP4' />
                      </div>
                      <span />
                      <div
                        ...
                        <video
                          ...
                          controls src='//images.dnatime.com/app/source/video/a7caf029-0592-40f0-a2dc-dceac42683b7.MP4' />
                      </div>
                    </div>
                  </div>
                </div>
