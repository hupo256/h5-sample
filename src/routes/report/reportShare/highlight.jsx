...
export default class ReportList extends Component {
    ...
    componentDidMount() {
        const { type, id, linkManId, productCategory, status, reportType, shareToken, code, barCode } = getParams()
    ...
    render() {
        const { categoryList, shareToken, highLightDesc,
            highLightTitle, QRlist, noScroll, loginVisible,tipsShow ,qnaireVerdict,traitNum} = this.state
                    ...
                    <div className={type == 'H' ? styles.hBanner : styles.lBanner} >
                        ...
                        {/* <div style={type == 'H' ? { backgroundImage: `url(${highlight})` } : { backgroundImage: `url(${lowlight})` }}
                            ...
                            <div className={styles.highLightTitle}>
                                ...
                                {/* <p>{highLightDesc}</p> */}
                                {traitNum?<h2>共{traitNum}个</h2>:null}
                            </div>
                        </div>
                    </div>
                    {qnaireVerdict?<div className={styles.highLightLink} >
                        <div className={styles.chart} style={{backgroundImage:`url(${qnaireVerdict.imageUrl})`}}>
                        {qnaireVerdict.btn}
                        </div>  
