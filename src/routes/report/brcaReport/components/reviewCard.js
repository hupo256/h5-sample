...
class ReviewCard extends Component {
    static propTypes = {
      ...
      componentDidMount() {
        ...
        this.setState({ isGet:data.isGet })
      }
      modalToggle = (name, couponId) => {
        ...
        if (this.state.isGet === 0) {
          brcaReportApi.userRreceiveCoupon({
            couponId,
            noloading:1
          }).then(res => {
            console.log(res);
        ...
        return (
          <div className={styles.padding15}>
            ...
            <div className={`${styles.square} ${styles.reviewCard}`} >
              ...
              <div className={`${styles.coupon} ${isGet === 1 ? styles.coupon2 : ''}`} onClick={() => this.modalToggle('haveBtn', data.couponId)}>
                <div className={styles.left}>
                  <label>¥</label><span>{data.amount}</span>
                </div>
