...
import { trackPointHpvReportDetailResultPageView, HpvReportDetailSciencePageView, ShareLinkGoto } from './buried-point'
const { getParams } = fun
@inject('hpvReport')
@observer
...
class report extends React.Component {
  ...
  pageInit = (res) => {
    ...
    if (res.data) {
      ...
      trackPointHpvReportDetailResultPageView({
        ...
        report_code:this.state.productCode,
        qnaire_status:this.state.qnaireStatus
      })
  ...
  reportDetailChangeIndex = index => {
    ...
    this.setState({
      ...
      !isAndall && ShareLinkGoto({
        page_code:tabIndex === 0 ? 'hpv_report_detail_science_page' : 'hpv_report_detail_result_page',
        sample_linkmanid: this.state.linkManId,
        report_code:this.state.productCode,
      })
    })
  ...
  render() {
    ...
    const { tabIndex, scienceList, resultList,
      ...
      isAndall, showTop, loading, isAnswer, QRlist, productId, isLatestReport } = this.state
