...
class HpvReportList extends Component {
  ...
  toDetail=(item, index1, index2) => {
    ...
    if(item.reportStatus=="1"||item.reportStatus=="2"){
      if (ua.isAndall()) {
        let url = `${window.location.origin}/mkt/sampling/hpv-status?barcode=${item.barcode}`
        window.location.href = `andall://andall.com/inner_webview?url=${url}`
      } else {
        this.props.history.push(`/sampling/hpv-status?barcode=${item.barcode}`)
      }
    }
    else{
      if (ua.isAndall()) {
                            ...
                            item1.hpvDtos && item1.hpvDtos.length &&
                              item1.hpvDtos.map((item2, index2) => (
                                <li key={index2} className={item2.reportLookStatus === 0 ? styles.isNew : item2.result === 1 ? styles.isRed : ''}>
                                  <div className={`${styles.my_ctn} ${styles.active}`}>
                                    {item2.reportStatus=="1"||item2.reportStatus=="2"?
                                    ...
                                    <div className={styles.check_btn} onClick={() => this.toDetail(item2, index1, index2)}>{item2.reportStatus=="1"||item2.reportStatus=="2"?'查看状态':'查看报告'}</div>
                                  </div>
                                </li>
                              ))
                          }
