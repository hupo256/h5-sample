...
class List extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      let productNamesDom = document.getElementsByClassName('productNames')
      const _list = []
      list.forEach((el, index) => {
        el.isMore = productNamesDom[index].clientHeight > 17
        _list.push(el)
      })
      this.setState({
        list: _list,
  ...
  handleSetContent = (productName, i, item) => {
    let productNamesDom = document.getElementsByClassName('productNames')
    console.log(productNamesDom)
    productNamesDom && productNamesDom.length > 0 && productNamesDom.forEach((item, index) => {
      console.log(item)
    })
    // if (productName.length > 14) {
    //   return (
    //     <i className='productNames'>
  ...
  handleSetClassName = (item) => {
    if (+item.btnStatus === 4) {
      return `${styles.btn} ${styles.btnDisable} ${styles.borderBlack}`
  ...
  render() {
    ...
    const { visible, item, showBCTips, list, oldList } = this.state
    const { linkManId, barcode } = item
    return (
      <div style={{ paddingBottom: btnStatus ? 135 : 75 }}>
        <ul className={styles.ul}>
          {list &&
            list.map((item, i) => (
              <li className={`white border flex sampling ${styles.list} ${+item.btnStatus === 4 ? styles.radioHide : ''}`} key={i}>
                <Check
                  value='1'
                  type='checkbox'
                  name={`shop${i}`}
                ...
                <div className={`item ${styles.content}`}>
                  ...
                  <p className={styles.itemProductName}>商品名称：
                    <i className='productNames'>
                      {console.log(oldList[i])}
                      <span className={`${styles.itemspan} ${oldList[i] && oldList[i].isOpen ? '' : styles.lineWord}`}>{item.productName}</span>
                      {/* ${item.isOpen ? '' : styles.lineWord} */}
                      {
                        item.isMore && item.isOpen
                          ? <img onClick={() => this.handleOpen(i, 'down')} src={images.iconUp} alt='' />
                          : ''
                      }
                      {
                        console.log(item.isMore)
                      }
                      {
                        item.isMore && !item.isOpen
                          ? <img onClick={() => this.handleOpen(i, 'up')} src={images.iconDown} alt='' />
