...
class Status extends React.Component {
  ...
  getSampleExceptionInfo = (num) => {
    ...
    samplingApi.getSampleExceptionInfo(obj).then(res => {
      const { code, data } = res
      if (!code) {
  ...
  handleCancle = () => {
    ...
    samplingApi.cancelRepeatSample(parmas).then(res => {
      const { code } = res
      if (!code) {
        // 确认放弃
  ...
  handleSubmit = (type) => {
    ...
    samplingApi.repeatSample(parmas).then(res => {
      const { code } = res
      if (!code) {
        ...
        <div className={styles.boxs}>
          ...
          <div className={styles.title}>异常样本处理</div>
          {
            info === 1 ? <div className={styles.samplingError}>
              <p className={styles.error}>样本无法进行检测，需要重新采集样本，请确认如下收件信息。（仅免费提供一次重新采样）</p>
