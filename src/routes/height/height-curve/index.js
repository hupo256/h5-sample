...
class HeightCurve extends React.Component {
  ...
  handleIsNewUser = () => {
    API.myInfo({ noloading:1 }).then(res => {
      ...
      if (data.checkMobileFlag === 2) {
        ...
        window.location.href = `${origin}/mobileLogin?url=height-curve`
      }
    })
  }
  handleQueryShare=() => {
  ...
  handleGoToRecordList = () => {
    ...
    this.props.history.push(`/height/record-list?linkmanId=${linkmanId}`)
  }
  // 制作折线图
  handleMarkLineChart = () => {
  ...
  handleGoToDetail = (item) => {
    ...
    window.location.href = `${window.location.origin}/mkt/news/article-detail-index?type=4&id=${id}`
  }
