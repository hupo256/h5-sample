...
class Curve extends React.Component {
  ...
  handleChangeTab = (index) => {
    ...
    if (index === 1 || index === 2 || index === 3) {
      if (!getSession('hasReport') || !getSession('evaluationFlag') || getSession('invalidFlag')) {
        const { bomb } = this.state
        if (getSession('invalidFlag')) {
          trackPointToolHeightOutPopup({
            sample_linkmanid: _linkmanId
          })
        }
  ...
  render () {
    const { tabsList, activedTab, title, mounthList, labelFlag, bomb, echartsData, curveDatas, desc } = this.state
        ...
        <div className={styles.curveDetail}>
          <div className={styles.curveDetail}>
            <p className={styles.curveTitle}>
              {
                activedTab === 0
                  ? <img onClick={this.handleMoreRecords} className={styles.moreRecords} src={images.iconMoreRecord} alt='' />
                  : ''
              }
            </p>
            {
        ...
        }
        {
          bomb.noDataBomb
            ? <NoDataBomb
              ...
              invalidFlag={getSession('invalidFlag')}
            />
            : ''
        }
      </div>
