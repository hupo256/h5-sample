...
const { getSession, getParams } = fun
class RecordList extends React.Component {
  ...
  handleEdit = (item) => {
    ...
    this.props.history.push(`/add-record?measureDate=${measureDate}&weight=${weight}&height=${height}&id=${id}`)
  }
  render () {
              ...
              ? list.map((item, index) => {
                return <div key={index} className={styles.item}>
                  ...
                  <div className={styles.rightCont}>
                    <img src={images.edit1} onClick={() => this.handleEdit(item)} alt='' />
                  </div>
                </div>
              })
              : ''
