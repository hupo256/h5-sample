...
import { API, fun, images, ua } from '@src/common/app'
...
class PredictedHeight extends React.Component {
  ...
  handleChoiceLinkman = (item) => {
    this.setState({
      ...
      if (status === 1) {
        this.props.history.push(`/height-index?linkManId=${linkManId}`)
        return
      }
      if (!hasReport) return this.props.history.push(`/userInfo`)
      this.setState({
        name: linkManName,
        bomb: {
          choiceLinkmanBomb: false
              ...
              ? <div className={styles.heightCont}>
                <div className={styles.choiceLinkman}>
                  <div className={styles.linkmansCont} onClick={this.handleChoiceLinkmanBomb}>
                    <img className={styles.sexImg} src={infos.sex === 'male' ? images.boy : images.girl} alt='' />
                    <span className={styles.name}>{name}</span>
                    {
                      linkmans.length > 1
                        ? <span className={styles.changeLinkMan}>
                          <img src={images.changeIcon} alt='' />
                        </span>
                        : ''
                    }
                  </div>
                ...
                <div className={styles.heightDetail}
                  style={{ background:`url(${images.indexBg}) no-repeat`, backgroundSize: '100%' }}
                >
                ...
                <div className={styles.footerCont}>
                  <div className={styles.detail}>
                    <div className={styles.intro}>
                      <img className={styles.user} src={images.user} alt='' />
                    ...
                    <p className={styles.tip}>
                      <img className={styles.img} src={images.tip} />
