  ...
  componentDidMount () {
    ...
    .then(res =>{
      ...
      if(status === 4 || status === 3){
        window.history.replaceState({activCode}, '', `/andall-sample/yunchan-succeed?activCode=${activCode}`);
        window.location.reload();
        return
      }
      if(status === 8) {
        window.history.replaceState({activCode}, '', `/andall-sample/yunchan-timeout`);
        window.location.reload();
        return
      }
