...
import { PickerView, DatePickerView, Toast, ImagePicker } from 'antd-mobile'
import { API, point, fun, ua } from '@src/common/app'
import { Page, Form } from '@src/components'
import { trackPointYmlxFreeOrdeView, trackPointYmlxFreeOrderGoto} from './buried-point'
import styles from '../mkt'

import yunctit from '@static/yunchan/yunctit.png'

const nowTimeStamp = Date.now()
const now = new Date(nowTimeStamp)
const { getSetssion, getParams, getDayNumber, creatTimeSelecter, fmtDate, isPoneAvailable } = fun
...
class EditOrder extends React.Component {
  ...
  toOrder = () => {
    ...
    trackPointYmlxFreeOrderGoto(pointPara)
    ...
    API.activOrderSubmit(paras).then(rs => {
      const {code, data, msg} = rs
      if(code) {
        Toast.fail(msg, 4);
        return
      }
      this.props.history.push(`/succeed?orderId=${rs.data}`);
    });
  }
  ...
  render () {
    const { expectDate, imageNameList, orderAddress, files, phoneNum, maxDate } = this.state;
    return (
      <Page title='安我助你平稳度过孕期'>
        <div className={styles.editOrder}>
          <img src={yunctit} alt='yunctit'/>

          <div className={`${styles.shippingFrom} shippingFrom`}>
          ...
          <div className='foot' onClick={this.toOrder} >
            <button 
