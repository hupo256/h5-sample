...
class MemberLand extends Component {
  ...
  handleGoToSubmit = () => {
    ...
    if (isAndall) {
      ...
      if (isOldUser === 1) { // 老用户
        window.location.href = `${window.location.origin}/mkt/orders/unlock-submit?buyType=5&flag=unlockLand&activeCode=${activeCode}`
      } else if (isOldUser === 2) { // 新用户
        seriesOrders.prodId = newUserProductId
        setSetssion('shopList', [seriesOrders])
        window.location.href = `${window.location.origin}/mkt/orders/order-submit?activeCode=${activeCode}`
      }
    }
  }
  handleIsNewUser = () => {
