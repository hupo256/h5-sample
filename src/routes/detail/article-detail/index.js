...
import { newmallarticleview } from '@src/routes/mkt/yinyang/componets/BuriedPoint.js'
...
export default class ArticleDetail extends React.Component {
  ...
  handleGoToExpert = (id) => {
    if (id) {
      this.props.history.push(`/expert-homepage?id=${id}`)
    }
  }
  handleGoTosan= (url) => {
    const { id } = getParams()
  ...
  render () {
    const { detail, isVideo, content, isGood, isAndall, isPlay, isArticle } = this.state
    let { title, displayTags, description, coverImgUrl, upNum, downNum, activContentExpertResp, contentCourseVideoUrl, goodsPictureUrl, describe, contentVideoUrl, creatTime, contentCourseVideoCoverPicture, contentVideoCoverPicture, upDownType, isShare, purchaseLink, isFollow } = detail || {}
          ...
          <div className={`${styles.header} ${isArticle ? styles.h283 : ''} ${isVideo ? styles.h206 : ''}`}>
            {
              isVideo
                ...
                : <div>
                  {
                    coverImgUrl || goodsPictureUrl ? <img className={styles.img} src={coverImgUrl || goodsPictureUrl} alt='' />
                      : ''
                  }
                </div>
            }
              ...
              isAndall
                ? <div className={styles.zanCont}>
                  <span onClick={() => this.handleAddActivUserUpDown(1)}>
                    <img src={upDownType === 1 ? images.zan : images.zanNo} alt='' />
