...
class SiweiBox extends React.Component {
  ...
  toTouchSpanPoint = (url, view, type) => {
    ...
    setTimeout(() => {
      ua.isAndall() && (location.href = `andall://andall.com/inner_webview?url=${url}`)
      ua.isAndall() || (location.href = url)
    }, 200)
  }
  ...
  gotoProduct = (obj) => {
    ...
    setTimeout(() => {
      ua.isAndall() ? 
        ...
        window.location.href = `${origin}/mkt/commodity?id=${productId}&linkManId=${linkManId}`
    }, 200)
  }
                    ...
                    return <li key={index}>
                      ...
                      <span>
                        {(geneValue || geneValue === 0) ?
                         <span style={{width: `${(geneValue/110)*100}%`}} /> : <img src={images.qicon} />
                        }
                      </span>
                    </li>
                  })
                    ...
                    return <li key={index}>
                      <span>
                        {(evaluationValue || evaluationValue === 0) ? 
                          <span style={{width: `${(evaluationValue/110*100)}%`}} /> : <img src={images.qicon} />
                        }
                      </span>
                      <div>
                        <img src={resultIconUrl || images.smile} />
                        <b>{tex}</b>
                      </div>
                    </li>
                  })
