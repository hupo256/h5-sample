...
export default function CheckOut({ history }) {
  ...
  function getDom() {
    return <div className={styles.checkoutBox}>
      {
        visitFlag === 0
  ...
  function goPage(page) {
    ...
    if (page === 1) {
      let url = window.location.origin + `/mkt/healthy/assessment?hideTitleBar=1`+`&linkManId=${getParams().linkManId}`
          ...
          ? <div className={styles.pullBox}>
            {
              puller({
                ...
                hasFirst:preEvaluationDto,
                hasEnd:sufEvaluationDto,
                callback
              })
            }
          </div>
