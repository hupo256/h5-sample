...
ajaxinstance.interceptors.request.use(
  request => {
    ...
    if (!data.noloading && !params.noloading) {
      loadinList.push(1)
      Toast.loading(data.loadingTit || params.loadingTit || '加载中...', 0)
    }
    token && (request.headers['Token'] = token) // (location.href = '/#/')
    request.headers['pageInfo'] = JSON.stringify(fun.getSession('pageInfo'))
    return request
  },
  error => {
    Toast.hide()
...
const API = {
  ...
  ...height(ajaxinstance),
  ...disease(ajaxinstance),
  ...product(ajaxinstance),
  ...report5(ajaxinstance),
}
